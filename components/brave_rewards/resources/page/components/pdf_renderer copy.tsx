...
const SignatureTypePopup: React.FC<SignatureTypePopupProps> = ({
  onClose,
  onConfirm
}) => {
      ...
      >
        ...
        {selectedSignature && (
          <div className={styles.selectedSignature}>
            <h3>
              {signatures.find((sig) => sig.id === selectedSignature.id)?.name}
            </h3>
            <p>{selectedSignature.expiry.toLocaleString()}</p>
...
const SignatureMethodPopup: React.FC<SignatureMethodPopupProps> = ({
  onClose,
  onSelectMethod
}) => (
  <div className={styles.popupOverlay}>
    <div className={styles.popupContent}>
      ...
      <div className={styles.methodOptions}>
        <div
          ...
          onClick={() => onSelectMethod('digitalID')}
        >
          <div className={styles.buttonTitle}>
            Sign with digital ID (Recommended)
          </div>
          <div className={styles.buttonDesc}>
            Sign documents quickly using your pre-uploaded signature data for a
            seamless and efficient signing process
          </div>
        </div>
        <div
          className={styles.button}
          onClick={() => onSelectMethod('imageUpload')}
        >
          <div className={styles.buttonTitle}>Upload Image Signature</div>
          <div className={styles.buttonDesc}>
            Select and upload an image of your signature from your device to
            sign documents easily and securely.
          </div>
        </div>
      </div>
    </div>
...
const SuccessPopup: React.FC<SuccessPopupProps> = ({
  ...
  <div className={styles.successPopup}>
    {isVerification ? (
      <h2
        className={`${styles.successTitle} ${styles.successVerificationTitle}`}
      >
    ...
    <div className={styles.successButtons}>
      <button
        className={`${styles.confirmButton} ${styles.btn}`}
        onClick={onSave}
      >
        Save as
      </button>
      <button
        className={`${styles.confirmButton} ${styles.continue}`}
        onClick={onContinue}
      >
        Continue
      </button>
...
export function PdfRenderer() {
  ...
  const [showSignatureMethodPopup, setShowSignatureMethodPopup] =
  ...
  const [showTypeSignaturePopup, setShowTypeSignaturePopup] =
    useState<boolean>(false)
  ...
  const showEmbedSignConfirmation = (pageIndex: number) => {
    const confirmation = window.confirm(
      'Do you want to embed the signature in the selected area?'
    )
    if (confirmation) {
  ...
  const handleMouseDown = useCallback(
    ...
    [overlayCanvasRefs, isSelectionEnabled]
  )
  ...
  const handleMouseMove = useCallback(
    ...
    [isSelecting, isSelectionEnabled, overlayCanvasRefs]
  )
  ...
  const drawSelection = useCallback(
    (endX: number, endY: number, pageIndex: number) => {
    ...
    [selectionCoords, overlayCanvasRefs]
  )
  ...
  const sendSignRequest = useCallback(async () => {
  ...
  }, [pdfBuff, currentPageIndex, selectedSignature, selectionCoords])
      ...
      <div className={styles.dropZoneContainer}>
        ...
        >
          ...
          <p className={styles.legalText}>
            ...
            <a
              href='#'
              style={{ color: '#2BB563', textDecoration: 'none' }}
            >
              {' '}
              Privacy policy{' '}
            </a>
  ...
  return (
    <div className={styles.app}>
      <header className={styles.header}>
          ...
          {pdfBuff && !isSelectionEnabled ? (
            <div className={styles.headerControls}>
              {isStatusVisible ? (
                ...
                <div
                  className={`${styles.fadeAway} ${
                    isStatusVisible ? styles.fadeAnimation : ''
                  }`}
                >
                  <button
                  ...
                  <button
                    ...
                    } ${isVerificationFailed ? styles.notVerified : ''}`}
            ...
            <div className={styles.headerControls}>
              <div className={styles.instructionText}>
                Start by holding right click and drag
              </div>
              <div className={styles.headerControlsBar}></div>
              <div className={styles.pageChangingControls}>
                <div
        ...
        <button className={`${styles.headerButton} ${styles.helpButton}`}>
          ?
        </button>
      </header>
      <div className={styles.pdfContainer}>
        {!pdfBuff ? (
          ...
          >
            <Document
              file={new Blob([pdfBuff], { type: 'application/pdf' })}
